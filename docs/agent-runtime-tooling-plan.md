...
```text
host data/runs/{run_id}/workspace  -> container /workspace
...
const shell = shellTool({
  name: "shell",
  shell: new DockerShellExecutor({ containerId, cwd: "/workspace" }),
  needsApproval: false,
});
```
...
const applyPatch = applyPatchTool({
  editor: new DockerWorkspaceEditor({ containerId, root: "/workspace" }),
  needsApproval: false,
});
```
...
- 把 workspace 当前版本中的指定文件复制到 Docker `/workspace`。
...
- 把 Docker `/workspace` 中的结果文件写回 WebCodex workspace。
...
2. 用 workspace_import 把需要修改或深入检查的文件导入 /workspace。
3. 用 shellTool 执行 rg、cat、python、curl、测试、构建等命令。
4. 用 viewTool2 查看长文本、图片、PDF 或二进制元数据。
5. 用 applyPatchTool 修改 /workspace 内文件。
...
- agent 可以用 `shellTool()` 在 Docker `/workspace` 中执行 bash。
- agent 可以通过 shell 执行 `curl`。
- agent 可以通过 shell 执行 `rg`，并且输出被截断保护。
- agent 可以用 `applyPatchTool()` 修改 `/workspace` 内文件。
...
- shell 不能访问宿主项目根目录。
- apply patch 不能写出 `/workspace`。
- viewTool2 不能读出 `/workspace`。
